...
Tunnel.prototype.init = function() {
  ...
  this.camera = new THREE.PerspectiveCamera(10, ww / wh, 0.001, 1);
...
Tunnel.prototype.addParticle = function() {
  ...
  // fontLoader.load('./fonts/OpenType-TT/Namastate_Bold.json', (font) => {
    for (var i = 0; i < 50; i++) {
      this.particles.push(new Particle(this.scene, false, 100, i, this.texture));
      this.texts.push(new Text(this.scene, true, 1000, this.font, "."))
...
Tunnel.prototype.render = function(time) {
  ...
  for(var i = 0; i < this.particles.length; i++){
    ...
    if(
      this.particles[i].burst && this.particles[i].percent > 1 
      && 
      this.texts[i].burst && this.texts[i].percent > 1
      ){
...
function Particle(scene, burst, time, i, texture) {
...
function Text (scene, burst, time, font, text) {
  ...
  if (!this.burst){
    this.speed *= 1;
    this.textMesh.scale.x *= 6.4;
    this.textMesh.scale.y *= 6.4;
    this.textMesh.scale.z *= 6.4;
  }
  // this.pos = new THREE.Vector3(0,0, global.iForPositionZ * 0.007)
...
Particle.prototype.plane = new THREE.PlaneBufferGeometry( 2, 2 );
...
Particle.prototype.update = function (tunnel) {
  ...
  if(global.scrollWhere === "down") {
    this.percent -= (this.speed - 1) * (this.burst ? 1 : tunnel.speed + 5)
  } else {
    this.percent += this.speed * (this.burst ? 1 : tunnel.speed + 5)
  }
...
Text.prototype.update = function (tunnel) {
  ...
  // this.textMesh.position.y = global.thisPos.y + 0.001;
  this.textMesh.position.y = 0.0048
...
window.onload = function() {
  ...
  textureLoader.load('/img/texture/Chunnel_All-7.png', (texture) => {
    fontLoader.load('./fonts/OpenType-TT/Namastate_Bold.json', (font) => {
      ...
      window.tunnel = new Tunnel(texture, font)
    })
  })
